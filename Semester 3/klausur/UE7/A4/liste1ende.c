...
int einfuegen_kopf(struct knoten* *kopfref, struct knoten* *endref, struct knoten* einzufueg) {
    if ((einzufueg==NULL)||(kopfref==NULL)||(endref==NULL)) return -1;
    einzufueg->next = *kopfref;
    *kopfref = einzufueg;
    if (einzufueg->next==NULL) *endref = einzufueg;
    return 0;
}


int einfuegen_nach(struct knoten* nachdiesem, struct knoten* einzufueg, struct knoten* *endref) {
    if ((nachdiesem==NULL)||(einzufueg==NULL)||(endref==NULL))  return -1;
...
struct knoten* entfernen_kopf(struct knoten* *kopfref, struct knoten* *endref) {
    struct knoten* kopf_alt;
    if ((kopfref==NULL)||(*kopfref==NULL)||(endref==NULL)) return NULL;
    kopf_alt = *kopfref;
    *kopfref = (*kopfref)->next;
    if((*kopfref)->next==NULL) *endref = *kopfref;
    return kopf_alt;
}


struct knoten* entfernen_ende(struct knoten* *kopfref, struct knoten* *endref) {
...
struct knoten* entfernen(struct knoten* *kopfref, struct knoten* *endref, struct knoten* auszufueg) {
    struct knoten* vor_auszufueg;
    if ((auszufueg==NULL)||(kopfref==NULL)||(*kopfref==NULL)||(endref==NULL)) return NULL;
