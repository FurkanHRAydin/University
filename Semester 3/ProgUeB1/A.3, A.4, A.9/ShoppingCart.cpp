...
void ShoppingCart::add(CartItem c) { //push_back erstellt kopie von kopie // besserer stil const ... &c 
    items.push_back(c);
}
...
int* ShoppingCart::getItemIds() {
    ...
    int* ids {new int[getNumbersOfItemns()]};

    for (int i = 0; i<getNumbersOfItemns(); i++) {
    
        ids[i] = items.at(i).getItemId();
    }
    return ids;
}
